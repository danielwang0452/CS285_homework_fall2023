...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            # clipped ensemble double q
            #indices = torch.randint(0, self.num_critic_networks, (2, batch_size))
            #next_qs = next_qs[indices, torch.arange(batch_size)]
            next_qs = next_qs.min(dim=0)[0]
        elif self.target_critic_backup_type == "mean":
            next_qs = next_qs.mean(dim=0)[0]
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward.to(torch.float32) + self.discount*(1-done.to(next_qs.dtype))*next_qs
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        """
        # sample actions and calculate negative log prob
        # TODO(student): Compute the entropy of the action distribution.
        # Note: Think about whether to use .rsample() or .sample() here...
        actions = action_distribution.rsample() # shape (batch, 1)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs.unsqueeze(0).repeat((self.num_actor_samples, 1, 1)), action) # broadcast obs across num_actor_samples
