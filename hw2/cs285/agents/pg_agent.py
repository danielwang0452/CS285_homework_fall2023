...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            critic_info: dict = self.critic
            for i in range(self.baseline_gradient_steps):
                info = self.critic.update(obs, q_values)
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = []
            for traj in rewards:
                r = self._discounted_return(traj)
                q_values.append(r)
            #print(q_values)
        else:
            ...
            q_values = []
            for traj in rewards:
                r = self._discounted_reward_to_go(traj)
                q_values.append(r)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            values = self.critic(torch.tensor(obs).to(device)).squeeze().cpu().detach().numpy()
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        gammas = np.array([self.gamma**t for t in range(rewards.shape[0])])
        return np.ones_like(rewards)*(gammas*rewards).sum()
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        out = np.ones_like(rewards)
        for t in range(rewards.shape[0]):
            gammas = np.array([self.gamma ** t for t in range(rewards[t:].shape[0])])
            out[t] = (gammas*rewards[t:]).sum()
        return out
'''
class PGAgent(nn.Module):
    def __init__(
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            # print("shape: ", q_values.shape, values.shape)
            assert values.shape == q_values.shape
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        # discounted_reward, discounted_factor = 0.0, 1.0
        # for i in range(len(rewards)):
        #     discounted_reward += rewards[i] * discounted_factor
        #     discounted_factor *= self.gamma
        # discounted_rewards = [discounted_reward for i in range(len(rewards))]
        ...
        #print(len(rewards), len(list(discounted_returns)))
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        #print(self.gamma)
        #print(rewards, list(discounted_rewards_to_go))
