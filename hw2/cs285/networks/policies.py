...
class MLPPolicy(nn.Module):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        action_distribution = self.forward(ptu.from_numpy(observation).to(device))
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(
        ...
        obs = ptu.from_numpy(obs).to(device)
        actions = ptu.from_numpy(actions).to(device)
        advantages = ptu.from_numpy(advantages).to(device)
