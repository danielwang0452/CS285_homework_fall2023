...
class DQNAgent(nn.Module):
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            target_values = reward.unsqueeze(-1) + torch.logical_not(
                done.unsqueeze(dim=1)) * self.discount * next_q_values
        ...
        q_values = qa_values.gather(dim=1,
                                    index=action.unsqueeze(dim=1))  # Compute from the data actions; see torch.gather
...
class DQNAgent(nn.Module):
    ...
    def update_critic(
        ...
        #print(q_values.shape, target_values.shape)
        loss = self.critic_loss(q_values, target_values) #+ self.cql_alpha*(qa_values.exp().sum(dim=1).log()-q_values).mean()
